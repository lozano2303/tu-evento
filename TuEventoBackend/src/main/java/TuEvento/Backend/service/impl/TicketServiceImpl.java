...
public class TicketServiceImpl implements TicketService {
    ...
    public ResponseDto<String> cancelTicket(int ticketID) {
        ...
        if (ticketOpt.get().getStatus() != 0)
            return ResponseDto.error("Ticket ya confirmado");
        Ticket ticket = ticketOpt.get();
        ticket.setStatus(1); // Confirmado
        ticketRepository.save(ticket);

        return ResponseDto.ok("Ticket confirmado con éxito");
    }
